...
function Tnm(e,p){return p.getElementsByTagName(e)}
...
TINY.table=function(){
	...
	sorter.prototype.init=function(){
		...
		for(i;i<t.w;i++){
			...
			if(d){
				var o=document.createElement('option'); o.value=i; o.innerHTML=Tnm('h3',c)[0].innerHTML; d.appendChild(o)
			}
		}
		this.reset()
	};
...
/* this references all the table rows:
var what=t.r[i].innerHTML;
window.alert(what);
*/
...
/* this alestrs Name row content because selected to sort by...
window.alert(t.a[i].v);
*/
		}
	...
	sorter.prototype.calc=function(){
		if(this.p.sum||this.p.avg){
			...
			if(!Tnm('tfoot',t)[0]){
				...
				f=Tnm('tfoot',t)[0]; while(f.hasChildNodes()){f.removeChild(f.firstChild)}
			}
			if(this.p.sum){
				r=this.newrow(f);
				for(i;i<t.w;i++){
...
*/
	...
	sorter.prototype.alt=function(){
		...
		for(i;i<t.l;i++){
			var r=t.r[i];
			if(t.a[i].s){
				r.className=x%2==0?this.p.evenclass:this.p.oddclass; var cells=Tnm('td',r);
	...
	sorter.prototype.page=function(s){
		...
		if(this.p.totalrecid){Tid(this.p.totalrecid).innerHTML=t.t}
		if(this.p.currentid){Tid(this.p.currentid).innerHTML=this.g}
		if(this.p.startingrecid){
			var b=((this.g-1)*this.p.size)+1, m=b+(this.p.size-1); m=m<t.l?m:t.t; m=m<t.t?m:t.t;
			Tid(this.p.startingrecid).innerHTML=t.t==0?0:b;; Tid(this.p.endingrecid).innerHTML=m
		}
...
//original		if(s<=this.d&&s>0){this.g=s; this.page((s-1)*this.p.size)}


//addon... 
s=parseInt(s); // fixes 1st issue
if(s<=this.d&&s>0){this.g=s; this.page((s-1)*this.p.size)}
Tid(this.p.pageddid).selectedIndex = this.g-1; // fixes 2nd issue - shows which page it is
	...
	sorter.prototype.size=function(s){
		...
		if(this.p.navid){Tid(this.p.navid).style.display=this.d<2?'none':'block'}
		this.page(0);
		if(this.p.totalid){Tid(this.p.totalid).innerHTML=t.t==0?1:this.d}
		if(this.p.pageddid){
			var d=Tid(this.p.pageddid), l=this.d+1;
	...
	sorter.prototype.search=function(f){
		var i=x=n=0, k=-1, q=Tid(f).value.toLowerCase();
		if(this.p.colddid){k=Tid(this.p.colddid).value}
	...
	sorter.prototype.set=function(){
		var t=Tid(this.id); t.b=Tnm('tbody',t)[0]; t.r=t.b.rows; this.t=t
	};
...
but doesn't work in IE6 - ie does not load page when page number selected!!!

sorter.prototype.goto=function(s){
s=parseInt(s); // fixes 1st issue
if(s<=this.d&&s>0){this.g=s; this.page((s-1)*this.p.size)}
Tid(this.p.pageddid).selectedIndex = this.g-1; // fixes 2nd issue
};
*/
